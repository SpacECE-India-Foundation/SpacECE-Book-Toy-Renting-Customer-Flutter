...
class _ServiceDetailsScreenState extends ConsumerState<ServiceDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final ProducServiceVariavtionDataModel variantFilter =
        ref.watch(variantfilterProvider);
    ...
    final ServiceVariavtionDataModel productFilter =
    ...
    if (productFilter.variantID == '') {
      ref.watch(servicesVariationsProvider).maybeWhen(
            orElse: () {},
            loaded: (_) {
              Future.delayed(buildDuration).then((value) {
                ref.watch(productfilterProvider.notifier).update((state) {
                  final List<Variant> variations = _.data!.variants!;
                  variant = _.data!.variants;
                  variations.sort(
            ...
            child: Column(
              children: [
                ...
                Container(
                  ...
                  child:
                      ref.watch(servicesVariationsProvider).map(initial: (_) {
                    return const LoadingWidget();
                  }, loading: (_) {
                    return const LoadingWidget();
                  }, loaded: (_) {
                    if (_.data.data!.variants!.isNotEmpty) {
                      return ListView.builder(
                        padding: EdgeInsets.zero,
                        shrinkWrap: true,
                        scrollDirection: Axis.horizontal,
                        itemCount: _.data.data!.variants!.length,
                        physics: const AlwaysScrollableScrollPhysics(),
                        itemBuilder: (context, index) {
                          return Padding(
                            padding:
                                EdgeInsets.only(left: index == 0 ? 16.h : 8.h),
                            child: GestureDetector(
                              ...
                              child: Container(
                                ...
                                child: Center(
                                  child: Text(
                                    _.data.data!.variants![index].name
                                        .toString()
                                        .toUpperCase(),
                ...
                if (variant != null)
                  Expanded(
                    child: SizedBox(
                      width: MediaQuery.of(context).size.width,
                      child: ref.watch(variationProductsProvider).map(
                        initial: (_) {
                          return const LoadingWidget();
                        },
                        loading: (_) {
                        ...
                        loaded: (_) {
                          if (_.data.data!.products!.isNotEmpty) {
                            return ListView.builder(
                              padding: EdgeInsets.zero,
                              ...
                              itemBuilder: (context, index) {
                                return Padding(
                                  ...
                                  child: ServiceDetailsContainer(
                                    product: _.data.data!.products![index],
                            ...
                            onTap: () {
                              if (!inCart) {
                                if (!canAddToCart) {
                                  ...
                                  Hive.box(AppHSC.appSettingsBox).put(
